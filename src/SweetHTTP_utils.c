...
void HTTP_sendHeaderResponse(const char *mineType, uint16_t responseCode, uint64_t size, const char *opcionais, struct SweetSocket_global_context *context, uint64_t id)
{
	...
	const char *connection = context->maxConnections == -1 ? "keep-alive" : "close";
	...
	uint64_t sizeHeader = strlen(headerHttp) + strlen(mineType) + strlen(status) + strlen(opts) + strlen(connection) + 30 + 21;
	...
	snprintf(headerToSend, sizeHeader, headerHttp, status, mineType, size, connection, date, opts);
...
size_t HTTP_createHtmlDirectoryList(const wchar_t *path, const wchar_t *virtualPath, wchar_t **html)
{
	...
	do
	{
		...
		if (HTTP_fileIsDirectory(pathName))
		{
			nameSize++;
