...
struct HTTP_server_config HTTP_loadConfig(wchar_t* confFile)
{
	...
	for (wchar_t* line = HTTP_readLine(configs.configFile); line != NULL; line = HTTP_readLine(configs.configFile))
	{
		...
		{
			if (wcsstr(value, L"Kb") != NULL) {
				configs.partialMaxFileBlock = _wtoi(value) * 1024;
				continue;
			}
			if (wcsstr(value, L"Mb") != NULL) {
				configs.partialMaxFileBlock = _wtoi(value) * 1024 * 1024;
				continue;
			}
	...
	if (configs.partialMaxFileBlock > (10 * 1024 * 1024))
		configs.partialMaxFileBlock = 10 * 1024 * 1024;
	return configs;
}
